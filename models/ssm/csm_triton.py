...
def triton_cross_scan(
    ...
    p_y2 = y + i_b * 4 * _tmp1 + _tmp1 + _tmp0 + i_w * BW * DH + tl.arange(0, BW)[None, :] * DH + i_h * BH + \
           tl.arange(0, BH)[:, None]  # trans
...
def triton_cross_scan_flex(
    ...
    HWRoute2 = (NH - i_h - 1) * BH * DW + (BH - 1 - tl.arange(0, BH)[:, None]) * DW + (NW - i_w - 1) * BW + (
                BW - 1 - tl.arange(0, BW)[None, :]) + (DH - NH * BH) * DW + (DW - NW * BW)  # flip
    HWRoute3 = (NW - i_w - 1) * BW * DH + (BW - 1 - tl.arange(0, BW)[None, :]) * DH + (NH - i_h - 1) * BH + (
                BH - 1 - tl.arange(0, BH)[:, None]) + (DH - NH * BH) + (DW - NW * BW) * DH  # trans + flip
...
class CrossScanTritonF(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        x = x.contiguous()
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_flex[(NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 0, 0, BC, BH, BW, C, H, W, NH,
                                                 NW)
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 1, 0, BC, BH, BW, C, H, W, NH,
                                                 NW)
        return x, None
...
class CrossScanTriton1b1F(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        x = x.contiguous()
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_flex[(NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 0, 1, BC, BH, BW, C, H, W, NH,
                                                 NW)
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](x, y, (0 if channel_first else 1), 0, 1, 1, BC, BH, BW, C, H, W, NH,
                                                 NW)
        return x, None
...
def triton_cross_scan_bidi(
    ...
    p_y3 = y + i_b * 4 * _tmp1 + 2 * _tmp1 + _tmp0 + (NH - i_h - 1) * BH * DW + (
                BH - 1 - tl.arange(0, BH)[:, None]) * DW + (NW - i_w - 1) * BW + (
                       BW - 1 - tl.arange(0, BW)[None, :]) + (DH - NH * BH) * DW + (DW - NW * BW)  # flip
    p_y4 = y + i_b * 4 * _tmp1 + 3 * _tmp1 + _tmp0 + (NH - i_h - 1) * BH * DW + (
                BH - 1 - tl.arange(0, BH)[:, None]) * DW + (NW - i_w - 1) * BW + (
                       BW - 1 - tl.arange(0, BW)[None, :]) + (DH - NH * BH) * DW + (DW - NW * BW)  # flip
