...
from models.ssm.ssm_global_model import GEncoder
...
class Local8x8_fuse_head(nn.Module):
    def __init__(self,
                 ...
                 activate=nn.Identity, zero_init=False):
    ...
    def forward(self, local_features, global_features):
        ...
        return fuse_superpixel, fuse_edge, fuse_feature # the pixel embeddings: torch.Size([2, 24, 208, 208])
...
class LEncoder(nn.Module):
    def __init__(self, Dulbrn=24):
        ...
        self.local_model = Backbone_VSSM(
            ...
            drop_path_rate=0.3,
            Dulbrn=Dulbrn)
        # The current code assumes that you are doing the second stage of the ssm training pipeline. This means that we load vmamba weights for the local
        # and load the author weights for the global model. The training here is more a finetune with the LAP. This should be ran with pipeline tv (only) with weights
        # to load set as tv_only.
        ckpt = "./weights/baselines/vssm_small_0229_ckpt_epoch_222_vmamba.pth"
        checkpoint = torch.load(ckpt, map_location='cpu')
        load_results=self.local_model.load_state_dict(checkpoint['model'], strict=False) # definitely strict=False since not using the classification part of the model
        # Expected missing keys
        expected_missing = {
            "npt_conv1.0.weight", "npt_conv1.0.bias",
            "npt_conv2.0.weight", "npt_conv2.0.bias",
            "outnorm0.weight", "outnorm0.bias",
            "outnorm1.weight", "outnorm1.bias",
            "outnorm2.weight", "outnorm2.bias"
        }
        # Verify only expected keys are missing
        unexpected_missing = set(load_results.missing_keys) - expected_missing
        if unexpected_missing:
            raise RuntimeError(f"Unexpected missing keys: {unexpected_missing}")
        else:
            print("there are unexpected keys in the LE state dict however this is wanted, we are not loading the entire classification model weights")
            print("the total number of missing keys is:", len(load_results.missing_keys))
            total_keys = len(self.local_model.state_dict())
            print(f"the total number of weights in the model is {total_keys}")
            print(f"Percentage missing: {len(load_results.missing_keys) / total_keys * 100:.2f}%")
            print(f"{load_results.unexpected_keys}")


        self.global_encoder = GEncoder(Dulbrn=24)
        # The global encoder is frozen when fine tuning
        global_ckpt= "./weights/baselines/model_best_global_model_ssm.tar"
        checkpoint = torch.load(global_ckpt, map_location='cpu')
        try:
            self.global_encoder.load_state_dict(checkpoint['state_dict'], strict=True)
            print("Global encoder checkpoint loaded successfully!")
        except RuntimeError as e:
            print(f"Error loading global encoder checkpoint: {e}")
            raise
    ...
    def forward(self, x):
        ...
        sup, edge, pix_embed = self.fuse_head(local_feature[0], global_feature[0])

        if self.training:
            return sup, edge, pix_embed
        else:
            return sup, pix_embed
