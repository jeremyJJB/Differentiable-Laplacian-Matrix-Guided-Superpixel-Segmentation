...
try:
    from models.ssm.csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1, getCSM
    from models.ssm.csm_triton import CrossScanTritonF, CrossMergeTritonF, CrossScanTriton1b1F
    from models.ssm.csms6s import CrossScan, CrossMerge
    from models.ssm.csms6s import CrossScan_Ab_1direction, CrossMerge_Ab_1direction, CrossScan_Ab_2direction, \
        CrossMerge_Ab_2direction
    from models.ssm.csms6s import SelectiveScanMamba, SelectiveScanCore, SelectiveScanOflex
    from models.ssm.csms6s import flops_selective_scan_fn, flops_selective_scan_ref, selective_scan_flop_jit
except:
    from models.ssm.csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1, getCSM
    from models.ssm.csm_triton import CrossScanTritonF, CrossMergeTritonF, CrossScanTriton1b1F
    from models.ssm.csms6s import CrossScan, CrossMerge
...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            ...
            # return SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, -1, -1, False)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        if self.mlp_branch:
            ...
            self.mlp = _MLP(in_features=hidden_dim,
...
class VSSM(nn.Module):
    ...
    def flops(self, shape=(3, 224, 224)):
        ...
        raise Exception("NEED FVcore if this method is used")
...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", Dulbrn=0, **kwargs):
        ...
        if self.Dulbrn:
            ...
            self.npt_conv1 = nn.Sequential(
                nn.Conv2d(3, self.Dulbrn, (3, 3), stride=1, padding=1),
                act
            )
            self.npt_conv2 = nn.Sequential(
                nn.Conv2d(self.Dulbrn, self.Dulbrn * 2, (3, 3), stride=2, padding=1),
                act
            )
            self.out_channels.extend([self.Dulbrn, self.Dulbrn * 2])
        self.out_channels.extend(self.dims[:-1])
